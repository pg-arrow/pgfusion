...
save_result_to_current() {
  [ -f "$SCRIPT_DIR/heatmap.html" ] && cp "$SCRIPT_DIR/heatmap.html" "$SCRIPT_DIR/checkpoints/current/heatmap.html"
  [ -f "$SCRIPT_DIR/queries.sql" ] && cp "$SCRIPT_DIR/queries.sql" "$SCRIPT_DIR/checkpoints/current/queries.sql"
}

if [ "$CHECKPOINT_ONLY" = "true" ]; then
  if [ ! -f "$SCRIPT_DIR/checkpoints/current/results.csv" ] && [ ! -f "$SCRIPT_DIR/checkpoints/current/results.json" ]; then
...
RESULTS_CSV="$SCRIPT_DIR/checkpoints/current/results.csv"
RESULTS_JSON="$SCRIPT_DIR/checkpoints/current/results.json"
...
log_info "Saving lastest resut heatmap to current"
save_result_to_current
