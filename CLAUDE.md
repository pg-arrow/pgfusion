# pgfusion Development Guidelines
...
├── datasource.rs     # CustomDataSource, CustomExec, SampleStream (DataFusion providers)
...
- `CustomExec` -- `ExecutionPlan` that partitions table reads across 10 parallel ranges
- `SampleStream` -- Bridges `futures::Stream` to DataFusion's `RecordBatchStream`
